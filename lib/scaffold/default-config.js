...
/**
 * Will return the path and default gamecore-node configuration. It will search for the
 * configuration file in the "~/.gamecredits" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.gamecredits');
  var defaultConfigFile = path.resolve(defaultPath, './gamecore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/gamecreditsd')
          }
        }
      }
    };
