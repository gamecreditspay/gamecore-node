...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'gamecore-lib': '^' + bitcore.version,
    'gamecore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/gamecore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the gamecore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the gamecore-node directory and configuration
