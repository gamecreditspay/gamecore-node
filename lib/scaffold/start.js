...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and gamecore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed gamecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './gamecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in gamecore-node services directory
